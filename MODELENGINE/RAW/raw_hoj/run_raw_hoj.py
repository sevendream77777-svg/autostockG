...
# -*- coding: utf-8 -*-
"""
raw_v52_fixed_full.py — FINAL (Original Features Preserved + Stability Patch)
- [Base] run_raw_hoj.py (매크로, 검증, 로깅 기능 100% 유지)
- [Fix 1] Pykrx 'market 옵션 오류' 방어를 위한 safe_pykrx_call 래퍼 적용
- [Fix 2] Naver Fallback 날짜 제한 해제 (count=3000 -> 30000)
- [Fix 3] 1개 종목 수집 시 32개 워커가 뜨는 비효율 수정 (Smart Worker Allocation)
"""
...
def atomic_write_csv(df: pd.DataFrame, path: Path):
    """
    파일 저장 도중 중단되어도 원본이 깨지지 않도록 보장합니다.
    .tmp 파일에 먼저 기록하고, 성공 시 원본 이름으로 교체(Atomic Rename)합니다.
    """
    ...
    try:
        ...
        # Windows의 경우 rename 시 타겟 파일이 있으면 에러가 발생할 수 있어 replace 사용
        ...
        print(f"[Critical] Atomic Save Failed ({path}): {e}")
        if tmp_path.exists():
            try:
                os.remove(tmp_path)
            except:
...
def calc_next_date(date_str: str) -> str:
    """YYYYMMDD -> 익일 YYYYMMDD"""
    try:
...
V36_COLS: List[str] = [
    ...
    # 2) 가격 & 상태 (9)
    "open", "high", "low", "close", "adjust_close", "volume", "amount", "adj_factor", "shares_out",
    # 3) 거래정지/상하한 (2)
    "is_trading_suspended", "is_limit_reached",
    # 3) 수급 및 주체 (Flow & Sentiment) (9)
    "frgn_net_amt", "inst_net_amt", "nps_net_amt", "tust_net_amt", "dealer_net_amt",
    "frgn_net_qty", "inst_net_qty", "nps_net_qty", "frgn_hold_ratio",
    # 5) 매크로/이벤트 (9)
...
# --------------------------------------------------------------------------- #
# [FIX] Pykrx 호출 래퍼 (안전장치 추가)
# --------------------------------------------------------------------------- #
def safe_pykrx_call(func, *args, **kwargs):
    """
    Pykrx 호출 시 발생하는 'market 옵션 오류'나 일시적 차단을 방어하기 위한 래퍼
    """
    max_retries = 3
    ...
    for i in range(max_retries):
        try:
            ...
            # 특정 에러 메시지는 즉시 재시도, 아니면 대기
            err_msg = str(e).lower()
            if "market" in err_msg or "json" in err_msg or "decode" in err_msg:
                # KRX 서버가 차단했거나 응답이 깨진 경우
                sleep_time = (i + 1) * 2 + random.uniform(0, 1)
...
def _collect_price_pykrx(code: str, start: str, end: str) -> pd.DataFrame:
    ...
    df = safe_pykrx_call(stock.get_market_ohlcv_by_date, start, end, code)
...
def _collect_price_fallback(code: str, start: str, end: str) -> pd.DataFrame:
    ...
    url = f"https://api.finance.naver.com/siseJson.naver?symbol={code}&requestType=1&timeframe=day&count=30000"
    ...
    df = df.sort_values("date")
    # [FIX] Amount 근사치 계산 (fallback 사용 시 최소한의 데이터 확보)
...
def collect_meta_and_cap(code: str, start: str, end: str) -> pd.DataFrame:
    ...
    try:
        # [FIX] Use safe_pykrx_call
        name = safe_pykrx_call(stock.get_market_ticker_name, code)
        df["name"] = name if name else str(code)
    except Exception:
        df["name"] = None
    # 시장 구분
    mk = None
    ...
    try:
        # [FIX] Use safe_pykrx_call
        mc = safe_pykrx_call(stock.get_market_cap_by_date, start, end, code)
...
class VectorizedCollector:
    ...
    def collect(self) -> pd.DataFrame:
        ...
        fetch_start = self.global_start
        existing_df = pd.DataFrame()
        
        if self.file_path and self.file_path.exists():
            try:
                # 기존 데이터 로드
                existing_df = pd.read_csv(self.file_path, dtype={'code': str, 'date': str})
                if not existing_df.empty and 'date' in existing_df.columns:
                    last_date = existing_df['date'].max()
                    if last_date >= self.global_end:
                        # 이미 최신 데이터까지 있음 -> 수집 건너뛰기
                        return existing_df
                    fetch_start = calc_next_date(str(last_date))
            except Exception as e:
                # 파일 깨짐 등 오류 시 기존 데이터 무시하고 새로 시작 (안전장치)
                print(f"[Warn] 기존 파일 로드 실패({self.code}): {e}, 새로 수집합니다.")
                existing_df = pd.DataFrame()
        
        # 2. 필요한 구간이 없으면 기존 데이터 리턴
        if fetch_start > self.global_end:
            return existing_df

        # 3. 신규 데이터 수집
        df_price = collect_price(self.code, fetch_start, self.global_end)
        if df_price is None or df_price.empty:
            # 신규 데이터가 없으면 기존 데이터만 반환
            return existing_df
            
        df_flow = collect_flow(self.code, fetch_start, self.global_end)
        df_meta = collect_meta_and_cap(self.code, fetch_start, self.global_end)
        ...
        # 5. 기존 데이터와 합치기 (Concatenate)
        final_df = df
        if not existing_df.empty:
            # 컬럼 스키마 맞추기
            for c in df.columns:
                if c not in existing_df.columns: existing_df[c] = pd.NA
        ...
        try:
            final_df["is_limit_reached"] = flag_limit_reached(final_df)
        except Exception:
            pass
        ...
        if self.file_path:
            atomic_write_csv(final_df, self.file_path)
...
def append_streaming(df: pd.DataFrame, out_dir: Path):
    ...
    # [NEW] 스트리밍 파일도 Atomic Write 적용 시도 (다만 Append 모드라 Lock 필요할 수 있음)
    # 여기서는 단순 Append 유지하되, 개별 파일이 보장되므로 보조 수단으로 활용
    try:
        with open(out_path, "a", encoding="utf-8-sig", newline="") as fout:
            df.to_csv(fout, header=header, index=False)
    except Exception as e:
...
def collect_parallel(
    ...
    # [FIX] Smart Worker Allocation (종목 수보다 워커가 많을 때 축소)
    actual_workers = min(len(tasks), num_workers)
    if actual_workers < 1: actual_workers = 1
    
    print(f"[병렬 수집] {len(tasks):,}개 종목, {actual_workers}개 워커 (Smart Allocation)")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="raw_v50_integrity_patch — 36 General Columns Collector")
