...
# -*- coding: utf-8 -*-
"""
raw_v48 ONEFILE (No-DART) — 36 General Columns Collector
- 단일 파일: run + collector 결합
- 재무(DART) 관련 기능/코드 전부 제거
- 36개 일반 컬럼만 수집 (가격/메타/수급/매크로/이벤트)
- 병렬 수집, 체크포인트, 품질 검증, 스트리밍 저장 지원
"""
...
# 경로 및 공용 설정
# --------------------------------------------------------------------------- #
ROOT = Path(__file__).resolve().parents[0]  # 단일 파일 기준
...
def safe_request(method: str, url: str, **kwargs) -> requests.Response:
    """프록시 + 기본 UA/언어 + 세션 재사용 + 백오프 + 레이트리밋"""
    proxy = get_proxy()
    if proxy:
        kwargs.setdefault("proxies", proxy)
    headers = kwargs.pop("headers", {}) or {}
    headers.setdefault("User-Agent", "Mozilla/5.0 (raw_v48 onefile nodart)")
...
def read_ecos_key() -> str:
    """ECOS API 키 로드 (있으면 사용, 없으면 fallback guest 키)"""
    paths = [
...
def _collect_price_pykrx(code: str, start: str, end: str) -> pd.DataFrame:
    ...
    df[["open", "high", "low", "close", "volume", "amount", "vwap"]] = (
        df[["open", "high", "low", "close", "volume", "amount", "vwap"]].ffill()
    )
    df["vwap"] = df["vwap"].fillna(df["close"])
    # adjust_close (단순 대용: close. 필요 시 외부보정 로직 삽입)
    df["adjust_close"] = df["close"]
    # 상태 기본값
    df["is_trading_suspended"] = False
    df["is_limit_reached"] = 0  # -1/0/1
    # shares_out / name / market 등은 메타에서 채움
...
def collect_flow(code: str, start: str, end: str) -> pd.DataFrame:
    ...
    try:
        ...
        if df_val is not None and not df_val.empty:
            ...
            if avail:
                result["inst_net_amt"] = df_val[avail].sum(axis=1)
...
def collect_meta_and_cap(code: str, start: str, end: str) -> pd.DataFrame:
    """메타(name/market/sector/shares_out 등) + 시총/상장주식수 보강"""
    try:
        # 종목 코드는 pykrx meta 호출에서 직접 사용하지 않지만, 필요 시 확장 가능
        # 여기서는 placeholder: shares_out/market_cap을 가격 조합으로 보강하도록 설계
        df = pd.DataFrame({"date": [start], "code": [code]})
        # 실제 메타 수집 루틴(예: 종목명/시장/섹터)은 사용자 환경 함수로 대체 가능.
        df["name"] = None
        df["market"] = None
...
def merge_macro(df: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    ...
    macro = pd.DataFrame({"date": df["date"].astype(str).copy()})
    macro = macro.drop_duplicates(subset=["date"])

    def _merge_one(key: str) -> Optional[pd.DataFrame]:
        # 우선순위: ECOS/FRED/yf 혼합
    ...
    # earnings_date는 외부 소스가 다양: 기본 None
    ...
    def collect(self) -> pd.DataFrame:
        ...
        if df_meta is not None and not df_meta.empty:
            for col in df_meta.columns:
                if col in ("date",):  # date는 의미 없음(단일행)
                    continue
                val = df_meta[col].iloc[0]
                df[col] = df.get(col, pd.NA)
                df[col] = df[col].fillna(val)

        # 시총/주식수 보강: shares_out/market_cap이 비거나 0이면 계산/보간
        if "shares_out" in df.columns and df["shares_out"].isna().any():
            df["shares_out"] = df["shares_out"].ffill().bfill()
...
# 병렬 수집
# --------------------------------------------------------------------------- #
def collect_one_stock(args: Tuple[str, str, str, Optional[str]]) -> Tuple[str, Optional[pd.DataFrame]]:
    code, start_date, end_date, proxy = args
    if proxy:
        try:
            set_proxy(proxy)
        except Exception:
...
def collect_parallel(
    ...
    tasks = [(code, start_date, end_date, proxy) for code in target_codes if not collector.is_completed(code)]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="raw_v48 ONEFILE (No-DART) — 36 General Columns Collector")
