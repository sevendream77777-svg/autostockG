...
def _classify_dart_status(dart_status: str) -> str:
    """Return one of: 'SUCCESS','NO_DATA','KEY_BLOCK','BAD_PARAM','RETRY'"""
    if dart_status == "000":
        return "SUCCESS"
    if dart_status == "013":  # 조회된 데이터가 없습니다.
        return "NO_DATA"
    # 인증키/쿼터 관련(키 교체가 의미 있음)
    if dart_status in {"300","320","334","336"}:
        return "KEY_BLOCK"
    # 파라미터 오류(키 교체로 해결되지 않음)
    if dart_status in {"100","101","102","103","104","020"}:
        return "BAD_PARAM"
    # 그 외 일시 오류는 재시도/로테이션
    return "RETRY"
# ============================ HTTP ============================
    ...
    while True:
        ...
        for _cycle in range(TOTAL_KEY_CYCLES):
            for _ in range(len(key_pool.keys)):
                ...
                last_resp = (status, data)

                # 즉시 로테이션 조건(페이지 증가 금지)
                # 분류 기반 처리
                if status in (429, 500, 502, 503, 504, 403) or status in (-1, 408) or data is None:
                    # 네트워크/서버 오류 → 키 로테이션
                    key_pool.rotate()
                    time.sleep(min(SLEEP_BACKOFF_MAX, SLEEP_BASE))
                    continue
                dart_status = str(data.get("status") or "")
                cls = _classify_dart_status(dart_status)
                if cls == "SUCCESS":
                    attempt_ok = True
                    # 아래에서 정상 처리
                elif cls == "NO_DATA":
                    # 이 페이지는 '정상적으로 비어 있음' → 키를 바꾸지 않고 페이지 종료 처리
                    attempt_ok = True
                elif cls == "KEY_BLOCK":
                    # 키 문제(만료/쿼터초과 등) → 즉시 로테이션, 페이지 증가는 금지
                    key_pool.rotate()
                    time.sleep(min(SLEEP_BACKOFF_MAX, SLEEP_BASE))
                    continue
                elif cls == "BAD_PARAM":
                    # 파라미터 문제 → 키 바꿔도 소용없음. 즉시 치명 오류로 중단.
                    print(f"[FATAL] 파라미터 오류(status={dart_status}). 요청 파라미터를 점검하세요.")
                    return  # 상위 루프에서 FAIL 마커를 기록하도록 함
                else:  # RETRY
                    key_pool.rotate()
                    time.sleep(min(SLEEP_BACKOFF_MAX, SLEEP_BASE))
                    continue
