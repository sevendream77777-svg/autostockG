# -*- coding: utf-8 -*-
"""
DART 공시 목록(list.json) 수집기 — 무결성 강화판 (SAFE)
...
def fetch_list_for_corp(
    ...
    # rows가 비었는데 여기로 왔다 → 013 외의 실패/타임아웃 등
    # 실패를 jsonl로 남김(이어받기 용)
...
def main():
    ...
    parser.add_argument("--no-skip-existing", action="store_true", help="이미 수집된 by_corp/zero/레거시 파일이 있어도 다시 수집")
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as ex:
        ...
        for fut in as_completed(fut_map):
            ...
            try:
                ...
                rec = {"corp_code": corp_code, "stock_code": c["stock_code"], "count": cnt, "issues": issues}
                summary.append(rec)
                if issues:
                    print(f"[DONE] {c['stock_code']} ({corp_code}) count={cnt} issues={issues}")
                    # 실패 기록(0건이지만 013은 기록 안 함)
                    if cnt == 0 and ("dart_status_013" not in issues):
                    ...
                    print(f"[DONE] {c['stock_code']} ({corp_code}) count={cnt}")
            except Exception as e:
                print(f"[ERROR] {c['corp_code']} 실패: {e}")
                rec = {"corp_code": c["corp_code"], "stock_code": c["stock_code"], "count": 0, "issues": [str(e)]}
                summary.append(rec)
                failures_fp.write(json.dumps({
