...
**최종 수정 사항 (Minimal Patch):**
1) UI 속도 완화: 기본 800ms (환경변수 UI_REFRESH_MS로 조절)
2) 텍스트 로그: 스크립트와 동일 루트에 run_raw_sle_ui.log 생성
3) Windows-safe 멀티프로세싱: 중첩함수 제거 + functools.partial 로 Pool.map 안정화

핵심 수집 로직 및 나머지 코드는 건드리지 않음.
"""
...
UI_LOG_PATH = SCRIPT_DIR / "run_raw_sle_ui.log"
...
def _ui_printer(_q, _ctr, _total):
    """표시 + 로그 동시 기록. Windows에서는 ANSI clear 사용 안 함(순간 넘김 방지)."""
    try:
        import sys
        buf: List[str] = []
        max_lines = 12
        # 화면 갱신 주기(ms). 환경변수 UI_REFRESH_MS 로 오버라이드 가능
        try:
            refresh_ms = int(os.environ.get("UI_REFRESH_MS", "800"))
        except Exception:
            refresh_ms = 800
        ...
        while True:
            ...
            if os.name != 'nt':
                # POSIX: 커서 홈 + 화면 클리어
                sys.stdout.write("\033[H\033[J")
            # Windows: 단순 출력(클리어 없음) → 순식간 스킵 방지
            ...
            # 로그에도 스냅샷 기록 (최근 10줄)
            _append_log([f"[SNAPSHOT] done={done} suc={suc} fail={fail}"] + buf[-10:])
            ...
            for k in ordered_keys:
                ...
                for fs_div in ["CFS", "OFS"]:
                    ...
                    if d_status == "000" and data.get("list"):
                        ...
                        break
                    elif d_status in ["020", "800"]: # Key Error -> swap key
                        data_status_code = d_status
                        break 
                    elif d_status == "013": # No Data -> Single API 백업
...
if __name__ == "__main__":
    ...
    if codes == [DEFAULT_CODE] and not args.codes and not args.codes_file:
        print(f"[WARN] 기본 코드로 대체: {DEFAULT_CODE}")
    ...
    _append_log([
        "===== EXECUTION START =====",
        f"END: {args.end}  START: {args.start}  WORKERS: {args.workers}",
        f"CODES: {len(codes)}",
        "-"*42
    ])
    ...
    try:
        if args.workers <= 1:
            for t in tasks:
                run_single_task(t, ctr, q)
        else:
            # Windows-safe: 중첩함수 없이 partial로 Pool.map
            func = partial(run_single_task, _ctr=ctr, _q=q)
            with mp.Pool(args.workers) as pool:
                pool.map(func, tasks)
    except KeyboardInterrupt:
        _append_log(["[INTERRUPTED] 사용자 강제 종료 감지"])
    finally:
        q.put(("STOP","."))
        ui_th.join()
        _append_log([
            ...
            f"TOTAL: {total}  SUCCESS: {int(ctr.get('success',0))}  FAIL: {int(ctr.get('fail',0))}",
            "-"*42
        ])
