...
- [Fix 4] 공공데이터포털 배당금 수집 무결성 강화 (필드명 변경 및 기업명 매핑 적용)
"""
...
def get_dividend_public_api(stock_name: str, base_date: str) -> Optional[float]:
    ...
    - Target: 보통주(Common Stock)의 현금배당금만 추출
    - Source: 한국예탁결제원(KSD) 최종 확정 데이터
    - Fix: 'se'(구분)가 None일 경우 'scrsItmsKcdNm'(주식종류명)을 확인하여 '보통주' 식별
    - Fix: 기업명 매핑(예: SK하이닉스 -> 에스케이하이닉스) 적용
    """
