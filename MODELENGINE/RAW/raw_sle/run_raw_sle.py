    ...
    for year in range(start_year, end_year + 1):
        ...
        for rc_idx, rc in enumerate(reprt_codes):
            ...
            for k in ordered_keys:
                ...
                # 2. 배당금 (별도 시도)
                # 재무제표를 찾았으면 그 키를 쓰고, 아니면 현재 루프의 키(k)를 쓴다.
                # [Fix: k는 loop 변수이므로 여기서 안전하게 사용 가능]
                cur_key = used_key if used_key else k
                dps = fetch_dividend_dps(corp_code, year, rc, cur_key)
                if dps is not None:
                    fin_row["div_amount"] = dps
                
                # 재무제표나 배당금 중 하나라도 건졌으면 다음 보고서로 넘어간다 (API 호출 절약)
                if got_fin or fin_row.get("div_amount") is not None:
                    break
