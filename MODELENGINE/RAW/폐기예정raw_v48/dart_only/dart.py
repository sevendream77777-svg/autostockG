...
# DART report code 매핑 (announce_date fallback 용)
ANN_REPORT_CODES = {
    "11011": "사업보고서",
    "11012": "반기보고서",
    "11013": "1분기보고서",
...
def rc_candidates(year: int, rc: str) -> List[str]:
    """
    2015년 분기(1Q/2Q/3Q)에서 공식 코드가 안 잡히는 경우를 대비해
    비공식 코드(11015, 11016, 11017, 11010)를 순차 시도.
    그 외 연도/연간은 기존 코드만 사용.
    """
    if year == 2015 and rc in {"11013", "11012", "11014"}:
        cands = [rc, "11015", "11016", "11017", "11010"]
        # 중복 제거 순서 유지
        seen = set()
        ordered = []
        for c in cands:
...
def collect_one_code(args):
    ...
    for year in range(start_year, end_year + 1):
        ...
        for rc in reprt_codes:
            ...
            for rc_try in rc_list:
                if rc_try != rc and VERBOSE:
                    print(f"      [fallback reprt_code 시도] {rc_try}")
                ...
                for key in ordered_keys:
                    announce_dt = fetch_announce_date(corp_code, year, rc_try, key)
                    if announce_dt:
                        break  # announce_date 확보됨

                if not announce_dt and VERBOSE:
                    print(f"        announce_date 없음 ({code6}, {year}, rc={rc_try})")
                ...
                for fs_div_opt in ("CFS", "OFS"):
                    ...
                    if st == "000" and data_all.get("list"):
                        ...
                        print(f"        [rc={rc_try} fs={fs_div_opt} api=All] status={st} msg={data_all.get('message')}")
                    if st == "020":
                        continue
                # 2) Singl: CFS → OFS
                if not got_fin:
                    for fs_div_opt in ("CFS", "OFS"):
                        ...
                        if st == "000" and data_s.get("list"):
                            ...
                            print(f"        [rc={rc_try} fs={fs_div_opt} api=Singl] status={st} msg={data_s.get('message')}")
                        if st == "020":
                            continue

                if got_fin:
                    break  # reprt_code 우회 성공 → 다음 rc로

            # --------------------------
            if got_fin and fin_row:
                if used_rc in seen_rc_for_year:
                    if VERBOSE:
                        print(f"      [중복] 이미 수집된 reprt_code={used_rc}, 스킵")
                    continue

                fin_row["corp_code"] = corp_code
                fin_row["bsns_year"] = str(year)
                fin_row["reprt_code"] = used_rc
                ...
                if used_rc == "11011":
                    ...
                    # fallback 코드(11015~17,11010)도 원래 분기 위치로 매핑
                    fin_row["date"] = f"{year}0331" if used_rc in {"11015"} else (
                        f"{year}0630" if used_rc in {"11016"} else (
                            f"{year}0930" if used_rc in {"11017"} else f"{year}1231"
                        )
                    )
                fin_row["code"] = code6
                collected_rows.append(fin_row)
                seen_rc_for_year.add(used_rc)
            else:
...
def finalize_one_code(code, collected_rows, out_dir, sample_csv_limit):
    ...
    # 기존 파일이 있으면 타임스탬프/증분 백업 후 덮어쓰기
    if csv_path.exists():
        ts = datetime.now().strftime("%Y%m%d%H%M%S")
        stem = csv_path.stem
        suffix = csv_path.suffix
        backup = out_csv_dir / f"{stem}_{ts}{suffix}"
        idx = 1
        while backup.exists():
            backup = out_csv_dir / f"{stem}_{ts}_{idx}{suffix}"
            idx += 1
        try:
            csv_path.rename(backup)
            print(f"[백업] {csv_path.name} -> {backup.name}")
        except Exception as e:
            print(f"[백업 실패] {e}")
