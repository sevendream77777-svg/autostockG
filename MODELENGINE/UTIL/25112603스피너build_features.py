...
def build_features(raw_dir, kospi_dir, feat_dir):
    ...
    sma_tp = tp.groupby(df["Code"]).rolling(20).mean().reset_index(0, drop=True)
    mad = tp.groupby(df["Code"]).transform(lambda x: x.rolling(20).apply(lambda y: np.mean(np.abs(y - np.mean(y)))))
    df["CCI_20"] = (tp - sma_tp) / (0.015 * mad).replace(0, 1e-6)
