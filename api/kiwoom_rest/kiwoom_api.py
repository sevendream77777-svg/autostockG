...
class KiwoomRestApi:
    ...
    def __init__(self):
        ...
        api_root = os.path.abspath(os.path.join(current_path, os.pardir))

        # config.ini: api/config.ini 우선, 없으면 로컬 config.ini 사용
        shared_config = os.path.join(api_root, "config.ini")
        local_config = os.path.join(current_path, "config.ini")
        self.config_path = shared_config if os.path.exists(shared_config) else local_config
        ...
        # TokenManager 초기화 (토큰은 로컬 유지)
        token_path = os.path.join(current_path, 'token.json')
        self.token_manager = KiwoomTokenManager(config_file=self.config_path, token_file=token_path)
    ...
    def _read_config(self):
        ...
        if not config_path:
            ...
            shared_config = os.path.join(api_root, "config.ini")
            local_config = os.path.join(current_path, "config.ini")
            config_path = shared_config if os.path.exists(shared_config) else local_config
