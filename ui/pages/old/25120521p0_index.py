                    ...
                    try:
                        ...
                        for item in lst:
                            ...
                            if "주당" in nm_norm:
                                if "eps" not in tmp:
                                    tmp["eps"] = val
                                    self._dart_map_logs.append(f"[MAP] eps <- {nm} ({aid}) = {val}")
                                continue
                            ...
                            if matched_key:
                                if matched_key == "op_income" and ("손익" in nm_norm) and ("영업" not in nm_norm):
                                    pass
                                else:
                                    tmp[matched_key] = val
                                    self._dart_map_logs.append(f"[MAP] {matched_key} <- {nm} ({aid}) = {val}")
