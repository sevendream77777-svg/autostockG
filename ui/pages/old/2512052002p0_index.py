...
class P0_Index(QWidget):
    ...
    def _fetch_dart_financial_robust(self, api_key: str, corp_code: str, base_year: int) -> Dict[str, Any]:
        ...
        # --- STRICT MAPPING (2014+ safe) ---
        # Collect logs and raw rows for debug UI
        self._dart_map_logs = []
        raw_rows = []

        # Whitelist-based IFRS/account_id tags (prefer parent-owner versions)
        IFRS_STRICT = {
            "revenue": [
                "ifrs-full_Revenue", "Revenue", "ifrs-full_RevenueFromContractsWithCustomers"
            ],
            "op_income": [
                "ifrs-full_ProfitLossFromOperatingActivities", "OperatingIncomeLoss"
            ],
            "net_income_owner": [
                "ProfitLossAttributableToOwnersOfParent", "ifrs-full_ProfitLossAttributableToOwnersOfParent"
            ],
            "net_income_total": [
                "ifrs-full_ProfitLoss", "ProfitLoss"
            ],
            "assets": ["ifrs-full_Assets", "Assets"],
            "liabilities": ["ifrs-full_Liabilities", "Liabilities"],
            "equity_owner": [
            ...
            "eps": [
                "ifrs-full_BasicEarningsLossPerShare", "EarningsPerShare"
            ],
            "cf_op": [
        ...
        # Helper: id/name contains any tag (case-insensitive)
        def _id_has(aid: str, tags):
            al = (aid or "").lower()
            return any(t.lower() in al for t in tags)

        def _nm_is(nm: str, targets):
            nm0 = (nm or "").replace(" ", "")
            return any(nm0 == t.replace(" ", "") for t in targets)

        def _nm_has(nm: str, substrs):
            nm0 = (nm or "").replace(" ", "")
            return any(s.replace(" ", "") in nm0 for s in substrs)

        # choose-first map with priority (do not overwrite once set)
        def _set_once(d, k, v, nm, aid):
            if k not in d:
                d[k] = v
                self._dart_map_logs.append(f"[MAP] {k} <- {nm} ({aid}) = {v}")
                ...
                for rc, _name in DART_REPORT_CODES:
                    try:
                        ...
                        for item in lst:
                            ...
                            raw_rows.append({"account_nm": nm, "account_id": aid, "value": val})

                            nm0 = nm.replace(" ", "")

                            # EPS guard: any '주당' 명시는 EPS로만 본다
                            if "주당" in nm0:
                                if _id_has(aid, IFRS_STRICT["eps"]) or _nm_has(nm, ["기본이익(손실)주당액", "주당순이익", "EPS"]):
                                    _set_once(tmp, "eps", val, nm, aid)
                                continue

                            # Revenue: only pure sales revenue, not cost/gross/other income
                            if _id_has(aid, IFRS_STRICT["revenue"]) or _nm_is(nm, ["매출액"]) or _nm_has(nm, ["수익(매출액)"]):
                                # exclude 매출원가/매출총이익/기타수익/금융수익 등
                                if not any(x in nm0 for x in ["매출원가", "매출총이익", "기타수익", "금융수익"]):
                                    _set_once(tmp, "revenue", val, nm, aid)
                                continue

                            # Operating income: require '영업' & '이익' (or IFRS op tag)
                            if _id_has(aid, IFRS_STRICT["op_income"]) or ("영업" in nm0 and ("이익" in nm0 or "손실" in nm0)):
                                _set_once(tmp, "op_income", val, nm, aid)
                                continue

                            # Net income: prefer owners-of-parent, else total ProfitLoss with strict names
                            if _id_has(aid, IFRS_STRICT["net_income_owner"]) or _nm_has(nm, ["지배주주순이익", "지배기업소유주지분순이익", "지배기업소유주에게귀속되는당기순이익"]):
                                _set_once(tmp, "net_income", val, nm, aid)
                                continue
                            if "net_income" not in tmp:
                                if _id_has(aid, IFRS_STRICT["net_income_total"]) and _nm_has(nm, ["당기순이익", "분기순이익", "반기순이익"]):
                                    # exclude 법인세비용차감전순이익
                                    if "법인세비용차감전" not in nm0:
                                        _set_once(tmp, "net_income", val, nm, aid)
                                        continue

                            # Balance sheet (prefer owners' equity)
                            if _id_has(aid, IFRS_STRICT["equity_owner"]) or _nm_has(nm, ["지배주주지분"]):
                                _set_once(tmp, "equity", val, nm, aid)
                                continue
                            if "equity" not in tmp and _id_has(aid, IFRS_STRICT["equity_total"]) or _nm_is(nm, ["자본총계"]):
                                _set_once(tmp, "equity", val, nm, aid)
                                continue

                            if _id_has(aid, IFRS_STRICT["assets"]) or _nm_is(nm, ["자산총계"]):
                                _set_once(tmp, "assets", val, nm, aid)
                                continue

                            if _id_has(aid, IFRS_STRICT["liabilities"]) or _nm_is(nm, ["부채총계"]):
                                _set_once(tmp, "liabilities", val, nm, aid)
                                continue

                            # Cash flows
                            if _id_has(aid, IFRS_STRICT["cf_op"]) or _nm_is(nm, ["영업활동현금흐름", "영업활동으로인한현금흐름"]):
                                _set_once(tmp, "cash_flow_op", val, nm, aid); continue
                            if _id_has(aid, IFRS_STRICT["cf_inv"]) or _nm_is(nm, ["투자활동현금흐름", "투자활동으로인한현금흐름"]):
                                _set_once(tmp, "cash_flow_inv", val, nm, aid); continue
                            if _id_has(aid, IFRS_STRICT["cf_fin"]) or _nm_is(nm, ["재무활동현금흐름", "재무활동으로인한현금흐름"]):
                                _set_once(tmp, "cash_flow_fin", val, nm, aid); continue
continue

                            # 한글 별칭 매칭
                            for std, aliases in DART_ACCOUNT_ALIASES.items():
                                if any(a.replace(" ", "") in nm.replace(" ", "") for a in aliases):
                                    tmp[std] = val
                                    break
...
if __name__ == "__main__":
    ...
    def show_dart_raw_table(self):
        d = self.last_raw_payloads.get("DART") or {}
        raw = d.get("_raw_list") or []
        logs = d.get("_map_logs") or []
        if not raw and not logs:
            self.log("[DART] raw/list 비어있음")
            return
        self.log("----- [DART RAW TABLE] -----")
        for r in raw[:500]:
            self.log(f"{r.get('account_nm')} | {r.get('account_id')} | {r.get('value')}")
        if len(raw) > 500:
            self.log(f"... ({len(raw)-500} more)")
        self.log("----- [DART MAP LOGS] -----")
        for line in logs:
            self.log(line)
        self.log("----- [END] -----")
