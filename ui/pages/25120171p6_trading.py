...
class TradingPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        top_splitter.setFixedHeight(350)
        ...
        mid_splitter.setFixedHeight(250)
    ...
    def _scan_files(self):
        ...
        for fpath in files:
            ...
            # 파일명에서 날짜 추출 (예: ..._251128.json)
            match = re.search(r"(\d{6})\.json$", fname)
            if match:
                date_str = "20" + match.group(1) # 20251128
                try:
                    qdate = QDate.fromString(date_str, "yyyyMMdd")
                    self.json_files_cache.append({"date": qdate, "path": fpath, "name": fname})
    ...
    def _on_date_clicked(self, date: QDate):
        ...
        matched = [f for f in self.json_files_cache if f["date"] == date]
        for m in matched:
