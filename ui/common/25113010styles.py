...
# common/styles.py
#
# 스킨 2종(Nord Dark / VSCode Black) + 커스텀(주요색/글자색) 지원 QSS 빌더
# - get_theme_nord_dark() / get_theme_black_vscode(): 베이스 팔레트 반환
# - build_qss(theme, primary, text): 전체 QSS 생성
# - get_nav_card_qss(theme): 좌측 NavCard 공용 스타일
#
# 주: 기존 UI 구조와 호환되도록 선택자/오브젝트명 유지
#    (예: #SideNav, #MainWindowContent, QLabel#NavSubtitle 등)
...
def _form_controls_qss(p):
    ...
    QLineEdit, QComboBox, QDateEdit {{
        ...
        border-radius: 6px;
        color: {p['text']};
        selection-background-color: {p['primary_alt']};
...
def get_nav_card_qss(p):
    ...
    QListWidget::item:selected {{
        background: transparent; /* 카드 자체에서 선택 표시 */
        border: none;
    }}
...
# QSS 빌드
# -------------------------------
def build_qss(theme: str, primary_color: str | None = None, text_color: str | None = None) -> str:
    # 테마 선택
    if theme == "black":
    ...
    # 사용자 지정 색 적용(옵션)
    if primary_color:
        pal["primary"] = primary_color
        pal["primary_alt"] = primary_color
    if text_color:
        pal["text"] = text_color
