...
# 실전용 HOJ 엔진 학습 스크립트 (HOJ_DB_REAL_V31 기준)
# ===========================================================
...
ENGINE_FILE = get_path("HOJ_ENGINE", "REAL", "HOJ_ENGINE_REAL_V31.pkl")
...
def train_real_engine():
    print("=== [REAL] HOJ 엔진 학습 시작 ===")
    print(f"  📥 입력 DB: {DB_PATH}")

    if not os.path.exists(DB_PATH):
        print("❌ 실전용 DB 파일이 없습니다. (build_HOJ_DB_REAL.py 먼저 실행 필요)")
        return
    ...
    for c in required_cols:
        if c not in df.columns:
            raise KeyError(f"❌ '{c}' 컬럼이 HOJ_DB_REAL_V31에 없습니다.")
